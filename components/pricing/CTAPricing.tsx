...
export function CTAPricing() {
  ...
  return (
    <section ref={ref} className="relative py-24 sm:py-32 overflow-hidden" aria-labelledby="pricing-cta-heading">
      <Container>
        <motion.div
        ...
        >
          <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
            ...
            <motion.div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[300px] rounded-full blur-[150px]"
              animate={breathe(!!shouldReduceMotion, 0.015, 0.04)}
              transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
              style={{ background: "radial-gradient(circle, var(--svg-cyan) 0%, var(--svg-link) 50%, transparent 70%)" }}
            />
            {!shouldReduceMotion && (
            ...
            {!shouldReduceMotion && isInView && <Particles count={8} speed={0.01} maxSize={0.5} />}
          </div>

          <div className="relative z-10">
            <motion.h3 initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, y: 16 }}
            ...
            >
              Ready to Scale Your{" "}
              <span className="bg-gradient-to-r from-accent-blue via-accent-violet to-accent-cyan bg-clip-text text-transparent">AI Future?</span>
            </motion.h3>

            <motion.p initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, y: 16 }}
              animate={isInView ? { opacity: 1, y: 0 } : undefined}
              transition={{ duration: 0.7, ease, delay: 0.4 }}
              className="text-base sm:text-lg text-text-secondary leading-relaxed max-w-xl mx-auto mb-8"
            >
              Book a free AI Strategy Session. We&apos;ll design a custom roadmap and
              provide ROI projections for your organization.
            </motion.p>

            <motion.div initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0, y: 16 }}
              animate={isInView ? { opacity: 1, y: 0 } : undefined}
            ...
            <motion.div initial={shouldReduceMotion ? { opacity: 1 } : { opacity: 0 }}
